...
class CliqueFPN():
    ...
    def __init__(self,num_classes,num_anchors,max_box_per_image,max_grid,ignore_thresh=0.6,learning_rate=0.0004):
    ...
    def loss(self,scale,x,anchors, max_grid,ignore_thresh):
        ...
        cell_x = tf.to_float(tf.reshape(tf.tile(tf.range(max_grid_w), [max_grid_h]), (1, max_grid_h, max_grid_w, 1, 1)))
        cell_y = tf.transpose(cell_x, (0,2,1,3,4))
        cell_grid = tf.tile(tf.concat([cell_x,cell_y],-1), [batch_size, 1, 1, self.num_anchors, 1])#max_grid的索引grid  (1,max_grid_h,max_grid_w,5,1)
